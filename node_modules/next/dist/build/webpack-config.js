...
entry:async()=>{return{...(clientEntries?clientEntries:{}),...(isServer&&hasDbModule?{'blitz-db':'./db/index'}:{}),...entrypoints};},watchOptions:{aggregateTimeout:5,ignored:['**/.git/**','**/node_modules/**','**/.next/**',// can be removed after https://github.com/paulmillr/chokidar/issues/955 is released
...
{}:{include:[dir,...babelIncludeRegexes]}),exclude:excludePath=>{if(babelIncludeRegexes.some(r=>r.test(excludePath))){return false;}return /node_modules/.test(excludePath);},use:hasReactRefresh?[require.resolve('@next/react-refresh-utils/loader'),defaultLoaders.babel]:defaultLoaders.babel},...(isServer?[]:[{issuer:/(mutations|queries)(?!.*\.client)/,resource:/_resolvers/,use:{loader:require.resolve('null-loader')}},// Because of packages/core/src/prisma-utils.ts
{test:/[\\/]npm-which[\\/]/,use:{loader:require.resolve('null-loader')}},{test:/[\\/]cross-spawn[\\/]/,use:{loader:require.resolve('null-loader')}}])].filter(Boolean)},plugins:[hasReactRefresh&&new _ReactRefreshWebpackPlugin.default(_webpack.webpack),// Makes sure `Buffer` and `process` are polyfilled in client-side bundles (same behavior as webpack 4)
...
version:`${"0.39.0"}|${configVars}`,cacheDirectory:_path.default.join(distDir,'cache','webpack')};// TODO - can we remove this?
...
profile:true}));webpackConfig.profile=true;}}}webpackConfig=await(0,_config.build)(webpackConfig,{rootDirectory:dir,customAppFile,isDevelopment:dev,isServer,assetPrefix:config.assetPrefix||'',sassOptions:config.sassOptions,productionBrowserSourceMaps:config.productionBrowserSourceMaps,future:config.future,isCraCompat:config.experimental.craCompat});let originalDevtool=webpackConfig.devtool;if(typeof config.webpack==='function'){webpackConfig=config.webpack(webpackConfig,{dir,dev,isServer,buildId,config,defaultLoaders,totalPages,webpack:_webpack.webpack});if(!webpackConfig){throw new Error('Webpack config is undefined. You may have forgot to return properly from within the "webpack" method of your blitz.config.js.\n'+'See more info here https://nextjs.org/docs/messages/undefined-webpack-config');}if(dev&&originalDevtool!==webpackConfig.devtool){webpackConfig.devtool=originalDevtool;devtoolRevertWarning(originalDevtool);}if(typeof webpackConfig.then==='function'){console.warn('> Promise returned in blitz config. https://nextjs.org/docs/messages/promise-in-next-config');}}if(!config.images.disableStaticImages&&_webpack.isWebpack5){if(!webpackConfig.module){webpackConfig.module={rules:[]};}const hasSvg=webpackConfig.module.rules.some(rule=>'test'in rule&&rule.test instanceof RegExp&&rule.test.test('.svg'));// Exclude svg if the user already defined it in custom
// webpack config such as `@svgr/webpack` plugin or
// the `babel-plugin-inline-react-svg` plugin.
webpackConfig.module.rules.push({test:hasSvg?/\.(png|jpg|jpeg|gif|webp|ico|bmp)$/i:/\.(png|svg|jpg|jpeg|gif|webp|ico|bmp)$/i,loader:'next-image-loader',dependency:{not:['url']},// @ts-ignore
issuer:{not:_css.regexLikeCss}});}if(config.experimental.craCompat&&(_webpackConfig$module2=webpackConfig.module)!=null&&_webpackConfig$module2.rules&&webpackConfig.plugins){// CRA prevents loading all locales by default
