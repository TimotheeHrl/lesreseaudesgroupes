...
'-h':'--help'};let args;try{args=(0,_index.default)(validArgs,{argv});}catch(error){if(error.code==='ARG_UNKNOWN_OPTION'){return(0,_utils.printAndExit)(error.message,1);}throw error;}if(args['--help']){console.log(`
    ...
    `);return;}const telemetry=new _storage.Telemetry({distDir:process.cwd()});let isEnabled=telemetry.isEnabled;if(args['--enable']||args._[0]==='enable'){telemetry.setEnabled(true);console.log(_chalk.default.cyan('Success!'));console.log();isEnabled=true;}else if(args['--disable']||args._[0]==='disable'){telemetry.setEnabled(false);if(isEnabled){console.log(_chalk.default.cyan('Your preference has been saved.'));}else{console.log(_chalk.default.yellow(`Blitz.js' telemetry collection is already disabled.`));}console.log();isEnabled=false;}else{console.log(_chalk.default.bold('Blitz.js Telemetry'));console.log();}console.log(`Status: ${isEnabled?_chalk.default.bold.green('Enabled'):_chalk.default.bold.red('Disabled')}`);console.log();if(isEnabled){console.log(`Blitz.js telemetry is completely anonymous. Thank you for participating!`);}else{console.log(`You have opted-out of Blitz.js' anonymous telemetry program.`);console.log(`No data will be collected from your machine.`);}console.log(`Learn more: https://nextjs.org/telemetry`);console.log();};exports.nextTelemetry=nextTelemetry;
