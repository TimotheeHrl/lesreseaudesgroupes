...
this.router.addFsRoute({match:(0,_router.route)(path),type:'route',name:`${path} exportpathmap route`,fn:async(req,res,_params,parsedUrl)=>{const{query:urlQuery}=parsedUrl;Object.keys(urlQuery).filter(key=>query[key]===undefined).forEach(key=>console.warn(`Url '${path}' defines a query parameter '${key}' that is missing in exportPathMap`));const mergedQuery={...urlQuery,...query};await this.render(req,res,page,mergedQuery,parsedUrl);return{finished:true};}});}}}async startWatcher(){if(this.webpackWatcher){return;}const regexPageExtension=(0,_utils3.buildPageExtensionRegex)(this.nextConfig.pageExtensions);let resolved=false;return new Promise((resolve,reject)=>{const pagesDir=this.pagesDir;// Watchpack doesn't emit an event for an empty directory
_fs.default.readdir(pagesDir,(_,files)=>{if(files!=null&&files.length){return;}if(!resolved){resolve();resolved=true;}});let wp=this.webpackWatcher=new _watchpack.default();wp.watch([],_utils3.topLevelFoldersThatMayContainPages.map(dir=>(0,_path.join)(pagesDir,dir)),0);wp.on('aggregated',()=>{const routedPages=[];const knownFiles=wp.getTimeInfoEntries();for(const[filePath,{accuracy}]of knownFiles){const relativePath='/'+(0,_path.relative)(pagesDir,filePath);if(accuracy===undefined||!(0,_utils3.isPageFile)(relativePath)||!regexPageExtension.test(filePath)){continue;}let pageName=relativePath.replace(/\\+/g,'/');pageName=(0,_utils3.convertPageFilePathToRoutePath)(pageName);pageName=pageName.replace(regexPageExtension,'');pageName=pageName.replace(/\/index$/,'')||'/';routedPages.push(pageName);}try{var _this$sortedRoutes;// we serve a separate manifest with all pages for the client in
