...
if(typeof nextConfig.exportPathMap!=='function'){if(!options.silent){Log.info(`No "exportPathMap" found in "blitz.config.js". Generating map from "./pages"`);}nextConfig.exportPathMap=async defaultMap=>{return defaultMap;};}const{i18n,images:{loader='default'}}=nextConfig;if(i18n&&!options.buildExport){throw new Error(`i18n support is not compatible with blitz export. See here for more info on deploying: https://nextjs.org/docs/deployment`);}if(!options.buildExport){const{isNextImageImported,isBlitzSessionMiddlewareUsed}=await nextExportSpan.traceChild('is-next-image-imported').traceAsyncFn(()=>_fs.promises.readFile((0,_path.join)(distDir,_constants2.EXPORT_MARKER),'utf8').then(text=>JSON.parse(text)).catch(()=>({})));if(isBlitzSessionMiddlewareUsed){throw new Error(`Blitz sessionMiddleware is not compatible with \`blitz export\`.
  Possible solutions:
    - Use \`blitz start\` to run a server, which supports middleware.
    - Remove \`sessionMiddleware\` import from \`blitz.config.js\`.\n`);}if(isNextImageImported&&loader==='default'&&!_ciInfo.hasNextSupport){throw new Error(`Image Optimization using Blitz.js' default loader is not compatible with \`blitz export\`.
  Possible solutions:
