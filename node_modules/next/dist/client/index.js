...
if(_utils.ST){performance.mark('beforeRender');}const reactEl=fn(shouldHydrate?markHydrateComplete:markRenderComplete);if(process.env.__NEXT_REACT_ROOT){if(!reactRoot){const createRootName=// blitz
typeof _reactDom.default.unstable_createRoot==='function'?'unstable_createRoot':'createRoot';reactRoot=_reactDom.default[createRootName](domEl,{hydrate:shouldHydrate});}reactRoot.render(reactEl);shouldHydrate=false;}else{// The check for `.hydrate` is there to support React alternatives like preact
