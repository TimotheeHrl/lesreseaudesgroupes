import { transformFiles, transform, rimraf } from '@blitzjs/file-pipeline';
import { pathExists, readFile, writeFile, readJSON, existsSync as existsSync$1, readJSONSync, outputFileSync, copy } from 'fs-extra';
...
function findNodeModulesRoot(src) {
  ...
  while (!includesBlitzPackage) {
    ...
    includesBlitzPackage = existsSync$1(join(nodeModules, "blitz"));
    count++;

    if (count > 5) {
      log.warning("We couldn't determine your actual node_modules location, so defaulting to normal location");
