...
var constants = require('../../dist/constants-98def2dd.cjs.prod.js');
var config = require('@blitzjs/config');
var display = require('@blitzjs/display');
var stdlib = require('next/stdlib');
...
require('../../dist/blitz-data-0b53eabc.cjs.prod.js');
require('htmlescape');
require('react');
...
async function paginate(_ref) {
  ...
  if (!isInteger(skip)) {
    throw new stdlib.PaginationArgumentError("`skip` argument must be a integer");
  }

  if (!isInteger(take)) {
    throw new stdlib.PaginationArgumentError("`take` argument must be a integer");
  }

  if (!isInteger(maxTake)) {
    throw new stdlib.PaginationArgumentError("`maxTake` argument must be a integer");
  }

  if (typeof countQuery !== "function") {
    throw new stdlib.PaginationArgumentError("`count` argument must be a function");
  }

  if (typeof query !== "function") {
    throw new stdlib.PaginationArgumentError("`query` argument must be a function");
  }

  if (skip < 0) {
    throw new stdlib.PaginationArgumentError("`skip` argument must be a positive number");
  }

  if (take < 0) {
    throw new stdlib.PaginationArgumentError("`take` argument must be a positive number");
  }

  if (take > maxTake) {
    throw new stdlib.PaginationArgumentError("`take` argument must less than `maxTake` which is currently " + maxTake);
  }
...
var SecurePassword = /*#__PURE__*/constants._extends({}, SecurePasswordLib__default['default'], {
  hash: async function hash(password) {
    if (!password) {
      throw new stdlib.AuthenticationError();
    }
...
var getDb = function getDb() {
  ...
  var path$1 = path.join(projectRoot, ".next/server/blitz-db.js");

  if (!fs__default['default'].existsSync(path$1)) {
    path$1 = path.join(projectRoot, ".next/serverless/blitz-db.js");
...
var defaultConfig = {
  ...
  createSession: function createSession(session) {
    ...
    return getDb().session.create({
      data: constants._extends({}, session, {
        userId: undefined,
        user: user
      })
    });
...
var sessionMiddleware = function sessionMiddleware(sessionConfig) {
  ...
  global.__BLITZ_SESSION_MIDDLEWARE_USED = true;
  global.sessionConfig = constants._extends({}, defaultConfig, sessionConfig); // Checks if cookie prefix from configuration has
...
var parseSessionToken = function parseSessionToken(token) {
  ...
  if (!handle || !id || !hashedPublicData || !version) {
    throw new stdlib.AuthenticationError("Failed to parse session token");
  }
...
var setCSRFCookie = function setCSRFCookie(req, res, antiCSRFToken, expiresAt) {
  ...
  assert$1(antiCSRFToken !== undefined, "Internal error: antiCSRFToken is being set to undefined");
...
async function getSessionKernel(req, res) {
  ...
  var enableCsrfProtection = req.method !== "GET" && req.method !== "OPTIONS" && req.method !== "HEAD" && !process.env.DANGEROUSLY_DISABLE_CSRF_PROTECTION;
  ...
  if (sessionToken) {
    ...
    if (version !== constants.SESSION_TOKEN_VERSION_0) {
      console.log(new stdlib.AuthenticationError("Session token version is not " + constants.SESSION_TOKEN_VERSION_0));
      return null;
    }
    ...
    if (enableCsrfProtection && persistedSession.antiCSRFToken !== antiCSRFToken) {
      ...
      throw new stdlib.CSRFTokenMismatchError();
    }
    /*
...
async function createNewSession(args) {
  ...
  if (args.anonymous) {
    ...
    var newPublicData = constants._extends({}, ((_args$jwtPayload = args.jwtPayload) == null ? void 0 : _args$jwtPayload.publicData) || {}, args.publicData);
    ...
    var newPrivateData = constants._extends({}, existingPrivateData, args.privateData);
...
async function refreshSession(req, res, sessionKernel, _ref2) {
  ...
  if ((_sessionKernel$jwtPay = sessionKernel.jwtPayload) != null && _sessionKernel$jwtPay.isAnonymous) {
    var payload = constants._extends({}, sessionKernel.jwtPayload, {
...
async function syncPubicDataFieldsForUserIfNeeded(userId, data) {
  ...
  if (Object.keys(dataToSync).length) {
    ...
    for (var _iterator = constants._createForOfIteratorHelperLoose(sessions), _step; !(_step = _iterator()).done;) {
      ...
      var publicData = JSON.stringify(constants._extends({}, session.publicData ? JSON.parse(session.publicData) : {}, dataToSync));
...
async function setPrivateData(sessionKernel, data) {
  ...
  var privateData = JSON.stringify(constants._extends({}, existingPrivateData, data));
  await global.sessionConfig.updateSession(sessionKernel.handle, {
    privateData: privateData
  });
}
async function setPublicData(req, res, sessionKernel, data) {
  ...
  var publicData = constants._extends({}, await getPublicData(sessionKernel), data);

  await refreshSession(req, res, constants._extends({}, sessionKernel, {
    publicData: publicData
  }), {
...
async function setPublicDataForUser(userId, data) {
  ...
  for (var _iterator3 = constants._createForOfIteratorHelperLoose(sessions), _step3; !(_step3 = _iterator3()).done;) {
    var session = _step3.value;
    // Merge data
    var publicData = JSON.stringify(constants._extends({}, JSON.parse(session.publicData || ""), data));
    await global.sessionConfig.updateSession(session.handle, {
      publicData: publicData
    });
  }
...
function passportAuth(config) {
  return async function authHandler(req, res) {
    ...
    var configObject = isFunction(config) ? config({
      ctx: res.blitzCtx,
      req: req,
      res: res
    }) : config;
    ...
    if (req.query.auth.length === 1) {
      ...
      middleware.push(connectMiddleware(passport__default['default'].authenticate(strategyName, constants._extends({}, authenticateOptions))));
 ...
 */
var fixNodeFileTrace = function fixNodeFileTrace() {
  var path = require("path");
...
var withFixNodeFileTrace = function withFixNodeFileTrace(fn) {
  fixNodeFileTrace();
  return fn;
};
