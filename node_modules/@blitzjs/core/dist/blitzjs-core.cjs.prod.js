...
var constants = require('./constants-98def2dd.cjs.prod.js');
...
var blitzData = require('./blitz-data-0b53eabc.cjs.prod.js');
...
var withRouter = function withRouter(WrappedComponent) {
  function Wrapper(_ref) {
    ...
    return /*#__PURE__*/React__default['default'].createElement(WrappedComponent, constants._extends({
      router: constants._extends({}, router, {
        query: query,
        params: params
      })
    }, props));
...
function useRouter() {
  ...
  return React__default['default'].useMemo(function () {
    return constants._extends({}, router, {
      query: query,
      params: params
    });
  }, [params, query, router]);
...
var ErrorBoundary = /*#__PURE__*/function (_React$Component) {
  ...
  function ErrorBoundary() {
    ...
    _this.handleRouteChange = function () {
      blitzData.clientDebug("Resetting error boundary on route change");
  ...
  _proto.componentDidCatch = async function componentDidCatch(error, info) {
    ...
    if (error instanceof stdlib.RedirectError) {
      var _this$context;

      blitzData.clientDebug("Redirecting from ErrorBoundary to", error.url);
...
ErrorBoundary.contextType = routerContext.RouterContext;
...
var useSession = function useSession(options) {
  ...
  if (options.initialPublicData) {
    initialState = constants._extends({}, options.initialPublicData, {
      isLoading: false
    });
  } else if (suspense) {
    if (blitzData.isServer) {
      ...
      initialState = constants._extends({}, publicDataStore.getData(), {
        isLoading: false
      });
    }
  } else {
    initialState = constants._extends({}, publicDataStore.emptyPublicData, {
      isLoading: true
    });
  }
  ...
  React.useEffect(function () {
    // Initialize on mount
    setSession(constants._extends({}, publicDataStore.getData(), {
      isLoading: false
    }));
    var subscription = publicDataStore.observable.subscribe(function (data) {
      return setSession(constants._extends({}, data, {
        isLoading: false
      }));
    });
...
var useAuthorizeIf = function useAuthorizeIf(condition) {
  if (typeof window !== "undefined" && condition && !publicDataStore.getData().userId) {
    var error = new stdlib.AuthenticationError();
    error.stack = null;
    throw error;
  }
};
var useRedirectAuthenticated = function useRedirectAuthenticated(to) {
  if (typeof window !== "undefined" && publicDataStore.getData().userId) {
    var error = new stdlib.RedirectError(to);
    error.stack = null;
    throw error;
  }
};
  ...
  var BlitzInnerRoot = function BlitzInnerRoot(props) {
    ...
    if (typeof window !== "undefined") {
      ...
      if (publicData.userId) {
        blitzData.clientDebug("[BlitzInnerRoot] logged in");
        ...
        if (redirectAuthenticatedTo) {
          var redirectUrl = typeof redirectAuthenticatedTo === "string" ? redirectAuthenticatedTo : utils.formatWithValidation(redirectAuthenticatedTo);
          blitzData.clientDebug("[BlitzInnerRoot] redirecting to", redirectUrl);
          var error = new stdlib.RedirectError(redirectUrl);
          error.stack = null;
          throw error;
        }
      } else {
        blitzData.clientDebug("[BlitzInnerRoot] logged out");
        ...
        if (authenticate && typeof authenticate === "object" && authenticate.redirectTo) {
          ...
          blitzData.clientDebug("[BlitzInnerRoot] redirecting to", url.toString());

          var _error = new stdlib.RedirectError(url.toString());
...
function withBlitzAppRoot(UserAppRoot) {
  var BlitzOuterRoot = function BlitzOuterRoot(props) {
    ...
    if (dehydratedState && _superjson) {
      var deserializedProps = SuperJSON__default['default'].deserialize({
        json: {
    ...
    return /*#__PURE__*/React__default['default'].createElement(BlitzProvider, {
      dehydratedState: dehydratedState
    }, noPageFlicker && /*#__PURE__*/React__default['default'].createElement(NoPageFlicker, null), /*#__PURE__*/React__default['default'].createElement(UserAppRoot, constants._extends({}, props, {
      Component: component
    })));
  };
...
function useQuery(queryFn, params, options) {
  ...
  var _useReactQuery = reactQuery.useQuery(constants._extends({
  ...
  var rest = constants._extends({}, queryRest, getQueryCacheFunctions(queryFn, params)); // return [data, rest as RestQueryResult<TResult>]
...
function usePaginatedQuery(queryFn, params, options) {
  ...
  var _useReactQuery2 = reactQuery.useQuery(constants._extends({
    queryKey: routerIsReady ? queryKey : ["_routerNotReady_"],
    queryFn: routerIsReady ? function () {
      return enhancedResolverRpcClient(params, {
        fromQueryHook: true
  ...
  var rest = constants._extends({}, queryRest, getQueryCacheFunctions(queryFn, params)); // return [data, rest as RestPaginatedResult<TResult>]
...
function useInfiniteQuery(queryFn, getQueryParams, options) {
  ...
  var _useInfiniteReactQuer = reactQuery.useInfiniteQuery(constants._extends({
  ...
  var rest = constants._extends({}, queryRest, getQueryCacheFunctions(queryFn, getQueryParams), {
  ...
  var promise = window.fetch(router.addBasePath(apiUrl), {
    ...
    if (response.headers) {
      ...
      if (response.headers.get(constants.HEADER_SESSION_REVOKED)) {
        blitzData.clientDebug("Session revoked, clearing publicData");
        publicDataStore.clear();
        setTimeout(async function () {
          // Do these in the next tick to prevent various bugs like https://github.com/blitz-js/blitz/issues/2207
          blitzData.clientDebug("Clearing and invalidating react-query cache...");
      ...
      if (response.headers.get(constants.HEADER_SESSION_CREATED)) {
        blitzData.clientDebug("Session created"); // await queryClient.invalidateQueries("")

        setTimeout(async function () {
          // Do these in the next tick to prevent various bugs like https://github.com/blitz-js/blitz/issues/2207
          blitzData.clientDebug("Invalidating react-query cache...");
          await queryClient.cancelQueries();
          await queryClient.resetQueries();
        });
      }
      ...
      if (response.headers.get(constants.HEADER_CSRF_ERROR)) {
        var err = new stdlib.CSRFTokenMismatchError();
        err.stack = null;
        throw err;
      }
    }
...
var getApiUrlFromResolverFilePath = function getApiUrlFromResolverFilePath(resolverFilePath) {
  ...
  return blitzData.getBlitzRuntimeData().trailingSlash ? ensureTrailingSlash(url) : url;
};
...
exports.formatZodError = blitzData.formatZodError;
exports.getBlitzRuntimeData = blitzData.getBlitzRuntimeData;
exports.validateZodSchema = blitzData.validateZodSchema;
